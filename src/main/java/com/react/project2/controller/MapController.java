...
public class MapController {
    ...
    @GetMapping("/{category}")
    public List<StudyMarkerDTO> getMarkerLocation(@PathVariable("category") String category) {
        log.info("********** MapController GET /:category - category : {}", category);
        // Entity Category를 이용해서 해당 카테고리의 위치정보를 가져온다.
        // 이 정보를 이용해서 지도에 마커를 찍는다
        List<StudyMarkerDTO> studyMarkerByCategory = studyService.getStudyMarkerByCategory(category);
